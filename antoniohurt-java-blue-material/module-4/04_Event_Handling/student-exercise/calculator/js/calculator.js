...
let allNumberQuery = document.getElementsByClassName('number');
for(i =0; i< allNumberQuery.length; i++){
allNumberQuery[i].addEventListener('click', clickNumber);
}
...
document.getElementsByClassName('decimal')[0].addEventListener('click', clickNumber);

// get a reference to the all clear button
// add a click event listener to call the function clear
document.getElementsByClassName('all-clear')[0].addEventListener('click', clear);
...
allOperators[i].addEventListener('click', clickOperator);
}

// add click event listener for the equal sign
// should call the function performOperation

document.getElementsByClassName('equal-sign')[0].addEventListener('click', performOperation)
});