...
public class TodoControllerTest {
	...
	public void test_mapping_not_found() throws Exception {
		this.mockMvc.perform(get("/api/todos/foo/bar")).andExpect(status().isNotFound());
	}

	@Test
	public void test_list_todos() throws Exception {
		this.mockMvc.perform(get("/api/todos").accept(MediaType.APPLICATION_JSON_UTF8_VALUE)).andExpect(status().isOk())
				.andExpect(content().contentType("application/json;charset=UTF-8"))
				.andExpect(jsonPath("$.[0].id").value("1")).andExpect(jsonPath("$.length()").value(4)).andDo(print());
	}

	@Test
	public void test_that_valid_todo_is_found() throws Exception {
		this.mockMvc.perform(get("/api/todos/1").accept(MediaType.APPLICATION_JSON)).andExpect(status().isOk());
	}

	@Test
	public void test_that_invalid_todo_is_not_found() throws Exception {
		this.mockMvc.perform(get("/api/todos/999").accept(MediaType.APPLICATION_JSON)).andExpect(status().isNotFound());
	}
	...
	public void test_create_with_valid_body_is_created() throws Exception {
		this.mockMvc
				.perform(post("/api/todos").contentType(MediaType.APPLICATION_JSON_UTF8_VALUE)
	...
	public void test_delete_with_valid_id_success() throws Exception {
		this.mockMvc.perform(delete("/api/todos/4")).andExpect(status().isOk());
