...
SELECT region, governmentform, AVG(population), SUM(surfacearea), (AVG(surfacearea) / population) AS population_density
FROM country
WHERE population > 0
GROUP BY region, governmentform, population
ORDER BY region;
...
-- Find the names of cities with a population > 1,5000,000 in countries with a gnp > 1,000,000 and a population > 80,000,000
