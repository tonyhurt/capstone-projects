...
namespace Exercises.Tests
{
    public class PropertyValidator
    {
        internal static void ValidateReadOnly(PropertyInfo prop, string name, Type dataType)
        {
            ...
            Assert.IsTrue(prop.SetMethod.IsPrivate, $"{name} should not have a public setter.");
            Assert.IsTrue(prop.CanRead, $"{name} property needs a get accessor.");
            Assert.AreEqual(dataType, prop.PropertyType, $"{name} should be a(n) {dataType.Name}");
        }
